...
class UserFavArtistAlbumScrapper(Spider):
    ...
    def get_page_id_from_url(self, url):
        page_id = re.search(r'page=(.*)&query', url, re.M).group(1)
        return int(page_id)
    ...
    def parse(self, response):
        ...
        print(f"parsing user fav artists list page of page {self.cur_page}")
        ...
        for artist_card in artists_list_content:
            artist_page_req = artist_card.xpath('./a/@href').extract()[0]
            artist_page_url = f'https://www.xiami.com{artist_page_req}'
            yield Request(url=artist_page_url, headers=self.headers,
    ...
    def parse_artist_page(self, response):
        ...
        :return:
        """
        artist_name = response.xpath('//div[@class="artist-info"]/div/div/text()').extract()[0]
        print(f"parsing artist detail page of {artist_name}")
        artist_albums_page_req = \
        response.xpath('//div[@class="related-albums"]/div[@class="blocktitle"]/a/@href').extract()[0]
        artist_albums_page_url = f"https://www.xiami.com{artist_albums_page_req}"
        yield Request(url=artist_albums_page_url, headers=self.headers,
    ...
    def parse_artist_albums_page(self, response):
        ...
        for album_card_content in album_list_content:
            ...
            yield Request(url=album_page_url, headers=self.headers, callback=self.parse_album_page)
    ...
    def parse_album_page(self, response):
        ...
        print(f"parsing album detail page of {album_name}")
        ...
        for disc_info in discs_info:
            ...
            for song_info in songs_info:
                ...
                if tr_class != 'extended-row odd' and tr_class != 'extended-row even':
                    ...
                    try:
                        album_song['play_cnt'] = song_info.xpath('.//div[@class="playCount-container"]/text()').extract()[0]
